...
import RenewForm from "@/components/ExpiredLeases/RenewForm";
import RelinquishForm from "@/components/ExpiredLeases/RelinquishForm";

const PopupForm = () => {
  const dispatch = useDispatch();
  const popupFormValue = 'Renew'
  ...
  return (
    <AnimatePresence>
      {popupFormValue && (
        <motion.div
          className="fixed px-6 m-auto bottom-0 right-0 left-0 z-40 w-full max-w-screen min-h-screen flex justify-center items-center"
        ...
        >
          <button
            type="button"
            className="navbar-close absolute top-6 right-6 bg-primary h-12 w-12 rounded-full flex justify-center items-center z-50 t"
            onClick={closeModal}
          >
            <svg
